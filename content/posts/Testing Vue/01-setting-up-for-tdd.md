...
`vue-test-utils`는 뷰의 공식 테스트 라이브러리이고, 이 가이드를 진행하는 동안 사용할 것입니다. 이 라이브러리는 브라우저와 Node.js 환경 양쪽에서 모두 작동합니다. 그리고 다른 테스트 러너(test runner)와 함께 사용할 수 있습니다. 이 가이드에서는 Node.js 환경에서 작성한 테스트를 실행하려고 합니다.

`vue-cli`는 가장 간단하게 시작할 수 있는 방법입니다. 프로젝트를 설정해줄 뿐만 아니라 인기있는 테스트 프레임워크인 Jest도 설정해줍니다. 아래의 커맨드를 통해 설치해봅시다.

``` 
yarn global add @vue/cli
```

npm의 경우 아래와 같이 입력합니다.

``` 
npm install -g @vue/cli
```

`vue create [project-name]`을 실행해서 새로운 프로젝트를 만들어봅시다. "Manually select features"를 선택하고 "Unit Testing"과 test runner로 "Jest"를 선택합니다.

설치가 끝나고 나면, `cd`로 프로젝트에 들어가고 `yarn test:unit` 커맨드를 실행합니다.

전부 잘 작동한다면, 아래와 같은 메세지가 나옵니다.
...
축하합니다, 첫 번째 통과 테스트를 실행했습니다.

## 첫번째 테스트 작성하기

------

방금 전에는 프로젝트를 생성할 때 함께 있는 기존 테스트를 실행했습니다. 직접 테스트를 작성해봅시다. 컴포넌트를 작성하고, 작성한 컴포넌트를 테스트합니다. 전통적으로 TDD를 할 때, 실패하는 테스트를 먼저 작성하고 테스트를 통과하기 위한 코드를 작성하여 실행합니다. 지금은 컴포넌트를 먼저 작성하겠습니다.
...
터미널에서 `tarn test:unit`을 입력해서 테스트를 작동시킵니다. 파일이 `tests` 폴더에 있고 `.spec.js`로 끝나면 자동으로 테스트가 수행됩니다. 잘 작동됐다면, 아래와 같은 메세지를 보게됩니다.
...
마크업이 올바르게 돼있는 것을 볼 수 있고, 테스트는 통과했습니다. 테스트가 실패하지 않았기때문에 통과하고 있습니다. 사실 이 테스트는 절대 실패할 수가 없어서, 유용한 방법은 아닙니다. `Greeting.vue`를 변경하고 템플릿에서 `greeting`을 지워도 여전히 통과합니다. 그럼 한번 수정해보겠습니다.

## assertions 하기

------

컴포넌트가 올바르게 작동하는지 확인하기 위해서 assertion 하는게 필요합니다. Jest에서 제공하는 API인 `expect`를 사용해서 assertion을 할 수 있습니다. `expect(result).to [matcher] (actual)` 와 같은 형태로 작성합니다.

Matchers는 값과 객체를 비교하는 메서드입니다. 예를들면 아래와 같습니다.
...
`vue-test-utils`에는 마크업인 `wrapper.text`를 얻는 더 좋은 방법이 있습니다. 테스트의 마지막 부분을 마무리 해봅시다.
...
아주 좋아 보입니다. 그렇지만 정말로 잘 작동하는지 확인하려면, 항상 테스트를 실패하고나서 통과해야합니다. 전통적인 TDD에서는 실제로 기능하는 코드를 작성하기 전에 테스트 코드를 작성합니다. 테스트 코드의 실패를 보고, 실패 에러를 이용해서 코드를 어떻게 작성해야할지 도움을 받습니다. 이 테스트가 정말로 잘 작동하는지 확인해봅시다. `Greeting.vue`를 아래와 같이 수정해주세요.
